...
method (SSLv2/v3/TLSv1), options, verification settings, timeout settings. 
 ...
 # $ctx - value coresponding to openssl's SSL_CTX structure
 #
 # returns: value coresponding to openssl's SSL structure (0 on failure)
...
Copies the session structure fro $from to $to (+ also the private key and certificate associated with $from).
 ...
 # $to - value coresponding to openssl's SSL structure
 # $from - value coresponding to openssl's SSL structure
 #
 # returns: no return value
...
Gets X509 certificate from an established SSL connection.
 ...
 # $ssl - value coresponding to openssl's SSL structure
 #
 # returns: value coresponding to openssl's X509 structure (0 on failure)
...
Check openssl doc L<http://www.openssl.org/docs/ssl/SSL_get_current_cipher.html|http://www.openssl.org/docs/ssl/SSL_get_current_cipher.html>
and L<www.openssl.org/docs/ssl/SSL_CIPHER_get_name.html|www.openssl.org/docs/ssl/SSL_CIPHER_get_name.html>
...
In client mode, returns the list of client CAs sent from the server, if any.
 ...
 # $ssl - value coresponding to openssl's SSL structure
 #
 # returns: value coresponding to openssl's STACK_OF(X509_NAME) structure (0 on failure)
...
Returns the cipher actually used.
 ...
 # $ssl - value coresponding to openssl's SSL structure
 #
 # returns: value coresponding to openssl's SSL_CIPHER structure (0 on failure)
...
??? (does this function really work?)
 ...
 my $rv = Net::SSLeay::get_shared_ciphers($ssl, $buf, $len);
 # $ssl - value coresponding to openssl's SSL structure
...
Connects the BIOs $rbio and $wbio for the read and write operations of the TLS/SSL (encrypted) side of $ssl.
 ...
 # $ssl - value coresponding to openssl's SSL structure
 # $rbio - value coresponding to openssl's BIO structure
 # $wbio - value coresponding to openssl's BIO structure
 #
 # returns: no return value
...
for the chosen $ssl, overriding the setting valid for $ssl's SSL_CTX object.
 ...
 # $ssl - value coresponding to openssl's SSL structure
 # $list - value coresponding to openssl's STACK_OF(X509_NAME) structure
 #
 # returns: no return value
...
Sets a new TLS/SSL method for a particular $ssl object.
 ...
 # $ssl - value coresponding to openssl's SSL structure
 # $method - value coresponding to openssl's SSL_METHOD structure
 #
 # returns: 1 on success, 0 on failure
...
Compares ASN1_OBJECT $a to ASN1_OBJECT $b.
 ...
 # $a - value coresponding to openssl's ASN1_OBJECT structure
 # $b - value coresponding to openssl's ASN1_OBJECT structure
 #
...
Returns a copy/duplicate of $o.
 ...
 # $o - value coresponding to openssl's ASN1_OBJECT structure
 #
 # returns: value coresponding to openssl's ASN1_OBJECT structure (0 on failure)
...
Parses an extension and returns its internal structure.
 ...
 # $ext - value coresponding to openssl's X509_EXTENSION structure
 #
 # returns: pointer ???
...
confusing if the target is a muticharacter string type like a BMPString or a UTF8String.
 ...
 # $name - value coresponding to openssl's X509_NAME structure
...
Returns the certificate in ctx which caused the error or 0 if no certificate is relevant.
 ...
 # $x509_store_ctx - value coresponding to openssl's X509_STORE_CTX structure
 #
 # returns: value coresponding to openssl's X509 structure (0 on failure)
...
Adds X509 CRL $x into the X509_STORE $store.
 ...
 # $store - value coresponding to openssl's X509_STORE structure
 # $x - value coresponding to openssl's X509_CRL structure
 #
 # returns: 1 on success, 0 on failure
...
??? (more info needed)
 ...
 # $store - value coresponding to openssl's X509_STORE structure
 # $pm - value coresponding to openssl's X509_VERIFY_PARAM structure
 #
 # returns: 1 on success, 0 on failure
...
=item * X509_STORE_set_flags
 ...
 # $ctx - value coresponding to openssl's X509_STORE structure
...
Enables policy checking (it is disabled by default) and adds $policy to the acceptable policy set.
 ...
 # $param - value coresponding to openssl's X509_VERIFY_PARAM structure
 # $policy - value coresponding to openssl's ASN1_OBJECT structure
 #
 # returns: 1 on success, 0 on failure
...
??? (more info needed)
 ...
 # $to - value coresponding to openssl's X509_VERIFY_PARAM structure
 # $from - value coresponding to openssl's X509_VERIFY_PARAM structure
 #
 # returns: 1 on success, 0 on failure
...
Finds X509_VERIFY_PARAM by name.
 ...
 #
 # returns: value coresponding to openssl's X509_VERIFY_PARAM structure (0 on failure)
...
Any existing policy set is cleared. The policies parameter can be 0 to clear an existing policy set.
 ...
 # $param - value coresponding to openssl's X509_VERIFY_PARAM structure
 # $policies - value coresponding to openssl's STACK_OF(ASN1_OBJECT) structure
 #
 # returns: 1 on success, 0 on failure
...
Frees up the X509 structure.
 ...
 # $a - value coresponding to openssl's X509 structure
 #
 # returns: no return value
...
Returns X509_EXTENSION from $x509 based on given position/index.
 ...
 #
 # returns: value coresponding to openssl's X509_EXTENSION structure (0 on failure)
...
Return an X509_NAME object representing the issuer of the certificate $cert.
 ...
 # $cert - value coresponding to openssl's X509 structure
 #
 # returns: value coresponding to openssl's X509_NAME structure (0 on failure)
...
Adjust th ASN1_TIME object to the timestamp (in GMT).
 ...
 # $s - value coresponding to openssl's ASN1_TIME structure
...
Takes PEM file and loads all X509 certificates and X509 CRLs from that file into X509_LOOKUP structure.
 ...
 # $ctx - value coresponding to openssl's X509_LOOKUP structure
 # $file - (string) file name
 # $type - (integer) type - use constants &Net::SSLeay::FILETYPE_PEM or &Net::SSLeay::FILETYPE_ASN1
...
??? (more info needed)
 ...
 # $level - value coresponding to openssl's X509_POLICY_LEVEL structure
 # $i - (integer) index/position
 #
 # returns: value coresponding to openssl's X509_POLICY_NODE structure (0 on failure)
...
??? (more info needed)
 ...
 # $node - value coresponding to openssl's X509_POLICY_NODE structure
 #
 # returns: value coresponding to openssl's X509_POLICY_NODE structure (0 on failure)
...
??? (more info needed)
 ...
 # $node - value coresponding to openssl's X509_POLICY_NODE structure
 #
 # returns: value coresponding to openssl's STACK_OF(POLICYQUALINFO) structure (0 on failure)
...
??? (more info needed)
 ...
 # $tree - value coresponding to openssl's X509_POLICY_TREE structure
 #
 # returns: no return value
...
??? (more info needed)
 ...
 #
 # returns: value coresponding to openssl's X509_POLICY_LEVEL structure (0 on failure)
...
Free an allocated STACK_OF(X509_NAME) structure.
 ...
 # $sk - value coresponding to openssl's STACK_OF(X509_NAME) structure
 #
 # returns: no return value
...
Add a file of certs to a stack. All certs in $file that are not already in the $stackCAs will be added.
 ...
 # $stackCAs - value coresponding to openssl's STACK_OF(X509_NAME) structure
...
Returns name of the cipher used.
 ...
 # $cipher - value coresponding to openssl's SSL_CIPHER structure
 #
...
Returns the number of secret bits used for cipher.
 ...
 # $c - value coresponding to openssl's SSL_CIPHER structure
 #
...
It cannot be set for specific SSL_CTX or SSL objects.
 ...
 #
 # $cm - value coresponding to openssl's COMP_METHOD structure
 #
