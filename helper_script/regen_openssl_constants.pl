...
#      perl helper_script/regen_openssl_constants.pl -gen-c > t/local/20_autoload.t
#
# or you can generate pod doc section with all available constants:
#      perl helper_script/regen_openssl_constants.pl -gen-pod > temp-doc.pod
...
sub pod_doc {
  my @c = sort grep { $_ !~ /^_TEST_INVALID_CONSTANT$/ } map { $_->{name} } @{$_[0]};
...
MARKER
  for (@c) {
	$rv .= qq[like(eval('&Net::SSLeay::$_; "ok"') || \$\@, qr/^(ok|Your vendor has not defined SSLeay macro.*)\$/, "$_");\n];
  }
  return $rv;
}
...
if ($ARGV[0] && $ARGV[0] eq '-gen-c') {
  print "/* DO NOT EDIT THIS FILE - update __DATA__ section of helper_script/regen_openssl_constants.pl */\n\n";
  print MySubClass->C_constant({breakout=>~0,indent=>20}, @constants);
}
elsif ($ARGV[0] && $ARGV[0] eq '-gen-t') {
  print t_file(\@constants);
}
elsif ($ARGV[0] && $ARGV[0] eq '-gen-pod') {
  print pod_doc(\@constants);
}
else {
